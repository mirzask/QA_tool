...
# Safe to remove unused packages?
""" 
import numpy as np
...
def check_affine_fold(
    fold_root, save_csv_path
    ...
    for i in range(0, len(subj_list)):
        ...
        for j in range(len(sess_list)):
            ...
            for k in range(len(instance_list)):
                print(sess_path + "/" + instance_list[k])
                if os.path.exists(
                    sess_path + "/" + instance_list[k] + "/secondary"
                ) and not os.path.exists(sess_path + "/" + instance_list[k] + "/DICOM"):
                    os.rename(
                        sess_path + "/" + instance_list[k] + "/secondary",
                        sess_path + "/" + instance_list[k] + "/DICOM",
                    )
...
def dcm2nii_condition(
    SPORE_root, qa_csv
):  # this make sense for spore and mcl, and introduce condition for dcm2nii
    subj_list = os.listdir(SPORE_root)
...
def cp_to_combine(data_root, new_root, QA_csv):  # this copy the QA data to combine.
    ...
    for i, item in df.iterrows():
        if item["usability"] != item["usability"]:
            ...
            new_path = (
                new_root + "/" + str(item["subject"]) + "time" + str(item["time"])
            )
            mkdir(new_path)
